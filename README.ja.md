...
**v1.3.8 ローカルモデル Hotfix** — Ollama / ローカル OpenAI-compatible エンドポイントで、Studio の書籍作成、サービステスト、モデル一覧、CLI の書籍作成が空の API key で失敗する回帰を修正しました。ローカルの動的モデル名を再び書籍作成と執筆に利用できます。
