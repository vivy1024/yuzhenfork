...
- **密度不是靠切段落实现——是靠段落内的语义密度实现**：每段仍维持 3-5 行手机阅读长度（约 40-120 字），禁止把一句动作、一句台词、一句反应各自切成孤立短段堆砌节奏感。只有需要单独承重的"爆点短段"（一击命中、一句改变局势的台词、一个定格动作）才允许独立成段，一章里这类爆点短段总数不超过 5 个。审核会拒绝：100 段里 50+ 段 <35 字 / 连续出现 5 个以上 <35 字段落。
