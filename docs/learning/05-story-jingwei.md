...
**经纬（Jingwei）** — 二级结构：分区 → 条目。分区是分类容器（角色/势力/设定/伏笔/大纲/状态/规则），条目是具体内容（张三、青云宗、修炼体系）。

**条目格式** — Markdown 正文 + YAML frontmatter（标签、别名、自定义字段、关联章节）。

**AI 上下文参与** — 开启 `participatesInAi` 的条目在 AI 生成时自动注入上下文。配合可见性规则和 tokenBudget 精确控制注入范围。

**可见性规则** — 控制条目在哪些章节对 AI 可见：
- `global`：全局可见，适合核心世界观
