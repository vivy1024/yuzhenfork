...
**v0.4.0** | 2026-04-19 | 最后更新 2026-05-14 | 始终使用中文回复
...
### ⚠️ Superpowers 执行流程（强制）

**来源：obra/superpowers 方法论。解决的问题：同一个上下文里做完所有事导致偏离计划、跳过验证、自欺欺人。**
...
```
Implementer subagent 完成 → Spec 合规审查 → Code Quality 审查 → 标记完成
                                    ↓ 不通过              ↓ 不通过
                              修复后重新审查         修复后重新审查
```

- **Spec 合规审查**：代码是否完成了 spec 要求的所有内容？是否多做了 spec 没要求的？
- **Code Quality 审查**：代码质量是否合格？是否有 mock/占位/假数据？
...
- ❌ "我很有信心"（confidence ≠ evidence）
- ❌ "就这一次"（no exceptions）
- ❌ "Linter 通过了"（linter ≠ 功能验证）
- ❌ "测试通过了"（tests pass ≠ 用户体验正确）
...
| 功能完成门禁 | superpowers `feature-closure-gate` |
...
5. 复杂任务采用 Kiro spec 工作流：先写 `.kiro/specs/<feature>/requirements.md` 与 `design.md`，再生成 `tasks.md`
...
| 小说写作功能 spec | `.kiro/specs/novel-writing-features/` |
| 已完成预设/模式/工具 spec | `.kiro/specs/archive/writing-presets-v1/`、`writing-modes-v1/`、`writing-tools-v1/` |
