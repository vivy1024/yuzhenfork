...
### Requirement 12：提供商体系对标 NarraFork 三种 API 模式

NarraFork 的 OpenAI 兼容供应商支持三种 API 请求格式：
- **Completions**：GPT-4 及更老模型、国产模型（DeepSeek/Groq/Ollama）
- **Responses**：GPT-4o 及更新模型（OpenAI Responses API）
- **Codex**：从 Codex 反代出来的（支持思考强度、Fast Mode、WebSocket、多账号额度管理）

NovelFork 当前只有基础 CRUD，没有 API 模式选择。

验收标准：
- Provider 配置支持选择 API 模式（Completions / Responses / Codex）
- Codex 模式下显示额外配置：推理强度、Fast Mode、WebSocket、Account ID
- Completions/Responses 模式下隐藏 Codex 专属配置
- API 模式影响实际请求格式（Chat Completions API vs Responses API vs Codex 格式）

### Requirement 13：Codex 多账号与额度管理

NarraFork 支持多个 ChatGPT 账号用于 Codex 模型，有完整的额度管理系统。

验收标准：
- 支持添加多个 Codex 凭据（refresh_token 导入）
- 显示每个账号的 tier（Free/Plus/Pro/Team）和用量
- 额度总览：可用账号数、平均剩余、下次重置时间
- 负载均衡模式：Priority / Balanced / Tier Balanced
- Codex 专属推理强度（优先于全局默认）
- Fast Mode 开关（service_tier: priority）

### Requirement 14：浏览器截图验证
