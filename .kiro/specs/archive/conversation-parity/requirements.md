...
**现状**: FileChangesPanel 在对话区底部折叠，用户需要手动展开
**目标**: 当 Agent 执行写作相关工具时，右侧自动弹出文件修改面板，实时显示写入内容

**触发条件**:
- `candidate.create_chapter` 执行时
- `Write` 写入经纬/大纲/章节文件时
- `Edit` 修改章节内容时
- `questionnaire.submit_response` 写入经纬时

**行为**:
- 面板从右侧滑出（不覆盖对话区，而是分栏显示）
- 实时显示正在写入的文件内容（利用已有的 file-changes-tracker）
- 写入完成后面板保持打开，用户可手动关闭
- 面板内显示：文件路径 + 内容预览 + diff（如果是修改）

**实现方案**:
- 在 `ConversationSurface` 或工作台布局中添加右侧面板 slot
- 监听 tool_result 事件，当工具是写作类工具时自动展开面板
- 面板内容复用 `FileChangesPanel` 的数据源

### 6.2 目录白名单快捷按钮

**现状**: 添加目录白名单需要去设置页
**目标**: 状态栏右侧添加 📁+ 按钮，点击弹出当前工作目录选择，一键加入白名单

**实现方案**:
- 状态栏右侧（权限按钮旁边）添加 `FolderPlus` 图标按钮
- 点击后弹出 Popover：显示当前 session 的 workDir，一键"添加到白名单"
- 调用 settings API 更新 `toolAccess.directoryAllowlist`
- 添加成功后 toast 提示

### 6.3 驾驶舱/资源树 UI 优化

**现状**: 
- "书籍详情加载失败"错误（API 调用问题）
- 左侧资源树为空（数据未加载）
- 新书引导向导占据整个画布，布局不协调
- 整体视觉粗糙，间距/颜色/层次感不足
...
- 修复书籍详情加载失败的 bug
- 资源树正确显示章节/经纬/候选稿/草稿
- 驾驶舱卡片视觉优化（圆角/阴影/间距/图标）
- 新书引导向导与资源树共存（向导在画布区，资源树在左栏）

**实现方案**:
- 排查 `/api/books/:id` 接口为什么返回错误
- 检查资源树数据加载逻辑
- 优化 CockpitOverview 组件的卡片样式
- 确保向导完成后驾驶舱正确渲染
...
- Phase 6：Agent 写章节时右侧自动弹出内容预览；驾驶舱无报错且资源树正常显示
