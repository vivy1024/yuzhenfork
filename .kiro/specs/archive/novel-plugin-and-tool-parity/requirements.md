...
| 位置 | 问题 | 修正 |
|------|------|------|
| `api/routes/auth.ts` | 整个 OAuth2 PKCE 流程（101-216 行）依赖 Sub2API | 删除 OAuth2 相关代码，保留 launch token 基础认证 |
| 环境变量 `SUB2API_URL` | 不再需要 | 删除引用 |
...
| 磁盘文件名 | `story_bible.md` / `book_rules.md` / `author_intent.md` | 重命名为经纬体系命名（无存量数据，可直接改） |
...
| 全仓搜索 `inkos` | 任何残留引用 | 清理（除 git remote 和历史文档） |

### 不动的

- `agent-native-workspace.ts` 文件名 — 已是核心架构概念
- Git upstream remote — 标准 fork 实践
- `docs/99-历史归档/` 中的 InkOS 引用 — 历史记录保留
...
- Phase 5：全仓无 `"inkos"` 字符串（除 git remote 和历史文档），无 Sub2API 耦合代码，无 `truth` 命名残留
