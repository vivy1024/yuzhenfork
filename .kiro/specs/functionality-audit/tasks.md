...
| 推理强度 "M" | 弹出 Menu（关闭/低/中/高/超高/自动）→ PUT session | UI 完整但 `onUpdateReasoningEffort` 未传递 | 接通回调 |
| Fast Mode ⚡ | toggle → PUT session | UI 完整但 `onToggleFastMode` 未传递 | 接通回调 |
...
| 切换推理强度 | ✅ 真实（API 层） | 但 NarratorStatusBar 控件是死的 |
| 切换权限模式 | ✅ 真实（API 层） | 同上 |
| Fast Mode | ❌ 未实现 | 无 `service_tier` 传递逻辑 |
