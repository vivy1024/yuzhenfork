...
| 引导完成 → 资源树刷新 | **已修复** | onGuideComplete → reloadResources 回调链接通 |
| 书籍删除 | **已修复** | 管理页面删除，不再依赖右键菜单 |
| ePub 导出 | **降级** | 输出为单文件 HTML 而非标准 .epub 格式 |
| Word (docx) 导出 | **未实现** | 前端有按钮但后端无 docx 生成逻辑 |
| 行内续写 Tab 补全 | **已接通** | 章节/候选稿/草稿编辑器按 Tab 触发续写 |
| 消息右键菜单 | **已接通** | 回退/分叉/压缩/编辑重生成/删除全部可用 |
| Context Ring 可视化 | **已接通** | 环形图 + 压缩/清空 + 自动压缩（读取设置页阈值） |
